  ...
  module LunrJsSearch
    class Indexer < Jekyll::Generator
      def initialize(config = {})
        ...
        @lunr_path = File.exist?(gem_lunr) ? gem_lunr : File.join('source', @js_dir, File.basename(gem_lunr))
      ...
      def generate(site)
        puts "Lunr:", 'Creating search index...'
        ...
        items.each_with_index do |item, i|
          ...
          puts "Lunr:", (entry.title ? "#{entry.title} (#{entry.url})" : entry.url)
        end
        ...
        puts "Lunr:", "Index ready (lunr.js v#{@lunr_version})"
        ...
        if File.expand_path(site_js) != File.dirname(@lunr_path)
          ...
          puts "Lunr:", "Added JavaScript to #{@js_dir}"
